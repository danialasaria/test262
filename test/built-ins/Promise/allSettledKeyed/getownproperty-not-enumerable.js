...
asyncTest(function() {
  return Promise.allSettledKeyed(input).then(function(result) {
    ...
    assert.sameValue(result.a.status, "fulfilled");
    assert.sameValue(result.a.value, 1);
    assert.sameValue(result.c.status, "fulfilled");
    assert.sameValue(result.c.value, 3);
  });
});
